# streamlit_app.py  – Mise à jour M2 (Personal Catalogue) et Appairage client
...
# psutil est optionnel : uniquement pour la jauge RAM
try:
    import psutil
except ModuleNotFoundError:
    psutil = None

DEBUG_SAMPLE: int | None = None  # ex: 10_000 pour tester plus vite
...
def read_csv(buf: io.BytesIO) -> pd.DataFrame:
    """CSV robust: détecte encodage & séparateur."""
...
if page.startswith("Mise à jour M2 – Appairage"):
    ...
    if st.button("🚀 Générer appairage", key="run_cl"):
        ...
        merged = (
            new_df.merge(old_df[["Ref", "M2_ancien"]], on
