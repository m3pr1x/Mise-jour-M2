...
# ---------------------------------------------------------------- UI
st.set_page_config(page_title="Mise à jour M2", page_icon="🛠", layout="wide")
...
def uploader(prefix: str, lots: Dict[str, tuple[str, str, str]]):
    ...
    for (k, (title, lab_ref, lab_val)), c in zip(lots.items(), cols):
        with c:
            st.subheader(title)
            ups = st.file_uploader(
                "Déposer…", type=("csv", "xlsx"),
                accept_multiple_files=True, key=f"{prefix}_{k}_up"
            )
            if ups:
                fresh = 0
                for up in ups:
...
if page.startswith("Mise à jour M2 – PC"):
    st.header("Mise à jour M2 (Personal Catalogue)")
...
if page.startswith("Mise à jour M2 – Appairage"):
    ...
    if st.button("Générer appairage"):
        ...
        dstr = datetime.today().strftime("%y%m%d")
        st.download_button(
            "Télécharger appairage M2_famille.csv",
